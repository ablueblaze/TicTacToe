const gameBoard = (() => {
  ...
  // let gameBoardObject = {
    ...
    const winCondition = function(plays){
      ...
      for (let i = 0; i < winList.length; i++){
        if (
          winList[i][2] == plays[0]
          // winList[i][1] in plays &&
          // winList[i][2] in plays 
        ){
          console.log('test')
        }
      }
      // console.log(winList[0][1])
    }

    const player = function(name, score, plays){
      return {
        name,
        score,
        plays
  ...
  return {gameBoardObject, generateBoard, board, winCondition}
})();
